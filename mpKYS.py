...
clr.AddReference("MissionPlanner.Utilities")  # includes the Utilities class from MissionPlanner.Utilities import Locationwp
...
except socket.error as msg: #changed to have as
...
while 1:
    ...
    if cs.mode == 'MANUAL':
        ...
        Follower_lat = float_lat + (Long_Offset_meters * math.sin(brng)) + (
                    Lat_Offset_meters * math.cos(brng))  # rotates lat follower offset in relation to heading of leader
        Follower_long = float_lng - (Long_Offset_meters * math.cos(brng)) + (
                    Lat_Offset_meters * math.sin(brng))  # rotates long follower offset in relation to heading of leader
